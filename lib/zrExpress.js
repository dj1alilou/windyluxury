...
async function resolveDeliveryTerritoryIds(order) {
    ...
    const districtTerritoryId =
      isUuid(order.districtTerritoryId) || isUuid(order.communeId)
        ...
        : order.commune && isZrExpressConfigured()
          ? ((await searchTerritoriesByKeyword(order.commune)).find(
              (territory) =>
                territoryLevelMatches(territory, "commune") &&
                territory.parentId === order.wilayaId &&
                territoryMatchesName(territory, order.commune),
            )?.id ||
            (await searchTerritoriesByKeyword("")).find(
              (territory) =>
                territoryLevelMatches(territory, "commune") &&
                territory.parentId === order.wilayaId,
            )?.id)
  ...
  const districtResultsFromKeyword = await searchTerritoriesByKeyword(order.commune);
  const districtResults =
    districtResultsFromKeyword.length > 0
      ? districtResultsFromKeyword
      : await searchTerritoriesByKeyword("");
  const district =

...
async function resolveDeliveryTerritoryIds(order) {
  ...
  const territories = await getAllTerritories();
  const wilayaKeywords = getTerritorySearchKeywords(order.wilaya, order.wilayaId);
  ...
  if (isUuid(order.cityTerritoryId) && territories.length > 0) {
    ...
    return {
      ...
      districtTerritoryId: districtTerritory?.id || order.districtTerritoryId,
    };
  }

  if (isUuid(order.wilayaId) && territories.length > 0) {
    const wilayaTerritory = territories.find((territory) => territory.id === order.wilayaId);
    const districtTerritory =
    ...
    return {
      cityTerritoryId: wilayaTerritory?.id || order.wilayaId,
      districtTerritoryId: districtTerritory?.id || order.districtTerritoryId,
    };
  }
